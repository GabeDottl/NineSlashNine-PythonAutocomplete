...
class ReferenceAstNodeVisitor(ast.NodeVisitor):
  ...
  def generic_visit(self, node):
    ...
    if not isinstance(node, (_ast.Module, _ast.Assign, _ast.Import,
                             _ast.ImportFrom, _ast.FunctionDef, _ast.ClassDef,
                             _ast.arg, _ast.alias, _ast.Name, _ast.Attribute)):
    ...
    complete_name = join_names(self.current_context,
                               name)  #_complete_name(node, self.node_to_parent)
    ...
    if isinstance(node, (_ast.ClassDef, _ast.FunctionDef)):
      ...
      for module_name in module_members:
        module_names.insert(
            0, (module_name, join_names(name, module_name), 'Unknown'))
  ...
  def _find_matching_name(self, node):
    if isinstance(node, _ast.Attribute):
      ...
      name = node.id
    for name_list in (self.current_local_names, self.current_enclosing_names,
                      self.current_instance_names, self.current_global_names):
